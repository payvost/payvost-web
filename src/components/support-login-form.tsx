...
export function SupportLoginForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Step 3: Create session cookie and verify support role
      const response = await fetch("/api/auth/support-session", {
        method: "POST",
        headers: {
      ...
      // Step 4: Redirect to support dashboard
      router.push("/customer-support-W19KouHGlew7_jf2ds/dashboard")
      router.refresh()
    } catch (error: any) {
      ...
      if (error.code === "auth/invalid-credential") {
        ...
        setError("Access denied. You do not have support team privileges")
      } else {
  ...
  return (
    <div className="w-full max-w-md space-y-8 rounded-xl bg-white p-8 shadow-lg">
      <div className="text-center">
        <h2 className="text-3xl font-bold text-gray-900">Support Login</h2>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="support@payvost.com"
              disabled={isLoading}
            />
          </div>
      ...
      <div className="text-center text-xs text-gray-500">
        <p>Only authorized support team members can access this portal</p>
      </div>
    </div>
  )
}
