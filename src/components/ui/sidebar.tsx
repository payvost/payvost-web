...
import { PanelLeft, PiggyBank } from "lucide-react"
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-2 overflow-auto pt-2 group-data-[collapsible=icon]:overflow-hidden pt-2",
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="group"
      className={cn("relative flex w-full min-w-0 flex-col px-2 pt-0 pb-2", className)}
      {...props}
    />
  )
})
