...
export function HrLoginForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Step 3: Create session cookie and verify HR admin role
      const response = await fetch("/api/auth/hr-session", {
        method: "POST",
        headers: {
      ...
      if (error.code === "auth/invalid-credential") {
        ...
        setError("Access denied. You do not have HR admin privileges")
      } else {
  ...
  return (
    <div className="w-full max-w-md space-y-8 rounded-xl bg-white p-8 shadow-lg">
      <div className="text-center">
        <h2 className="text-3xl font-bold text-gray-900">HR Admin Login</h2>
