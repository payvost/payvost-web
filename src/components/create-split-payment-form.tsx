...
import { Trash2, Plus, Users, Percent, Baseline, ArrowLeft } from 'lucide-react';
...
interface CreateSplitPaymentFormProps {
    onBack: () => void;
}

export function CreateSplitPaymentForm({ onBack }: CreateSplitPaymentFormProps) {
  ...
  const allocated = participants.reduce((acc, p) => {
    ...
    if (p.splitType === 'percentage') {
      return acc + (totalAmount * (Number(p.value) || 0)) / 100;
    }
    return acc;
  }, 0);
        ...
        <CardHeader>
             <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <CardDescription>Divide a single payment among multiple recipients.</CardDescription>
                </div>
            </div>
        </CardHeader>
