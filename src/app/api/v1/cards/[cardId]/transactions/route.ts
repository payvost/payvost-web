...
export async function GET(req: NextRequest, context: { params: Promise<{ cardId: string }> }) {
  try {
    ...
    const response = await fetch(buildBackendUrl(`/api/v1/cards/${encodeURIComponent(cardId)}/transactions${req.nextUrl.search}`), {
      method: 'GET',
    ...
    console.error('GET /api/v1/cards/[cardId]/transactions proxy error:', error);
