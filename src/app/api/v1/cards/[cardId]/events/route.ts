...
export async function GET(req: NextRequest, ctx: { params: Promise<{ cardId: string }> }) {
  try {
    ...
    const response = await fetch(buildBackendUrl(`/api/v1/cards/${encodeURIComponent(cardId)}/events${req.nextUrl.search}`), {
      method: 'GET',
    ...
    console.error('GET /api/v1/cards/[cardId]/events proxy error:', error);
