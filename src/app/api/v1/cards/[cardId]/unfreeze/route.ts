...
export async function POST(req: NextRequest, context: { params: Promise<{ cardId: string }> }) {
  try {
    ...
    const response = await fetch(buildBackendUrl(`/api/v1/cards/${encodeURIComponent(cardId)}/unfreeze`), {
      method: 'POST',
    ...
    console.error('POST /api/v1/cards/[cardId]/unfreeze proxy error:', error);
