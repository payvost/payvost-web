...
export async function GET(
  ...
  try {
    ...
    // Fetch recent transactions for this user (if they exist in a subcollection).
    ...
    let kycIdType: string | undefined;
    let kycIdNumber: string | undefined;
    ...
    if (data.kycProfile) {
      ...
      if (profile.tiers) {
        if (profile.tiers.tier3?.status === 'approved') {
          kycLevel = 'Advanced';
          if (!userType || userType === 'Tier 1' || userType === 'Tier 2') userType = 'Tier 3';
          ...
          if (!userType || userType === 'Pending') userType = 'Tier 1';
        }
      }
      ...
      if (!kycIdType && profile.idType) kycIdType = String(profile.idType);
      if (!kycIdNumber && profile.idNumber) kycIdNumber = String(profile.idNumber);
    }

    if (!kycIdType && data.idType) kycIdType = String(data.idType);
    if (!kycIdNumber && data.idNumber) kycIdNumber = String(data.idNumber);
    ...
    // Do not return any transaction PIN material (legacy or otherwise).
    const customer = {
      id: userDoc.id,
    ...
    console.log(`OK: Successfully fetched customer ${id}`);
