...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const response = await fetch(buildBackendUrl(`/api/wallet/accounts/${id}/ledger${search}`), {
      method: 'GET',
      headers: {
    ...
    console.error(`GET /api/wallet/accounts/${params?.id}/ledger proxy error:`, error);
