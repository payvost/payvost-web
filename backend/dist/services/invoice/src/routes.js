...
const recurringInvoiceProcessor = new recurring_invoice_processor_1.default(prisma);
...
router.post('/invoices/recurring/process', async (req, res) => {
    try {
...
router.get('/invoices/recurring/stats', async (req, res) => {
    try {
        ...
        recurringSnapshot.docs.forEach((doc) => {
            const data = doc.data();
            const frequency = data.recurringFrequency;
...
router.get('/invoices/recurring/scheduler/status', async (req, res) => {
    try {
...
router.post('/invoices/recurring/scheduler/trigger', async (req, res) => {
    try {
        ...
        const result = await (0, scheduler_1.processRecurringInvoices)();
        res.json(result);
    }
    catch (error) {
