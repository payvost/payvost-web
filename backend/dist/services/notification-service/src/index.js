...
async function sendEmailViaService(params) {
    try {
        ...
        const response = await axios_1.default.post(`${EMAIL_SERVICE_URL}/single`, emailData, {
            headers: {
 ...
 */
app.post('/notify/kyc', async (req, res) => {
    try {
 ...
 */
app.post('/notify/business', async (req, res) => {
    try {
...
app.post('/notify/transaction', async (req, res) => {
    try {
        ...
        const templateName = status === 'success'
            ? 'transaction-success'
            : status === 'failed'
                ? 'transaction-failed'
                : 'transaction-success';
 ...
 */
app.post('/notify/payment-link', async (req, res) => {
    try {
 ...
 */
app.post('/notify/invoice', async (req, res) => {
    try {
        const { email, name, invoiceNumber, amount, currency, dueDate, businessName, downloadLink, type, // 'generated' | 'reminder' | 'paid'
         } = req.body;
        ...
        const templateName = type === 'generated'
            ? 'invoice-generated'
            : type === 'reminder'
                ? 'invoice-reminder'
                : 'invoice-paid';
        const subject = type === 'generated'
            ? 'New Invoice'
            : type === 'reminder'
