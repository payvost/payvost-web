...
exports.determineOptimalProvider = determineOptimalProvider;
const registry_1 = require("../providers/registry");
async function determineOptimalProvider(request) {
    const { currency, amount, sourceCountry, destinationCountry, paymentMethod } = request;
    // Get all providers that support this currency
    const eligibleProviders = registry_1.PaymentProviderRegistry.getProviderForCurrency(currency)
        .filter(provider => {
        // Check amount limits
    ...
    const scoredProviders = await Promise.all(eligibleProviders.map(async (provider) => {
        let score = 0;
        // Prefer local providers for domestic transactions
        if (sourceCountry === destinationCountry) {
            if ((sourceCountry === 'NG' && ['paystack', 'flutterwave'].includes(provider.name)) ||
        ...
        switch (provider.name) {
            case 'paystack':
                if (currency === 'NGN')
                    score += 8;
                if (['NG', 'GH'].includes(sourceCountry))
                    score += 5;
                break;
            case 'flutterwave':
                if (['NGN', 'KES', 'GHS'].includes(currency))
                    score += 8;
                if (['NG', 'KE', 'GH', 'ZA'].includes(sourceCountry))
                    score += 5;
                break;
            case 'stripe':
                ...
                break;
            case 'sepa':
                if (currency === 'EUR' && sourceCountry.match(/^(DE|FR|IT|ES)$/))
                    score += 15;
                break;
        }
