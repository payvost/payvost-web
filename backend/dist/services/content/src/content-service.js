...
class ContentService {
    ...
    async createContent(input) {
        const { slug, status = 'DRAFT', isPublic = true, allowComments = false, language = 'en', metaKeywords = [], tags = [], mediaIds = [], coAuthors = [], ...rest } = input;
    ...
    async listContent(filters = {}) {
        const { contentType, status, authorId, category, search, limit = 50, offset = 0, } = filters;
        const where = {};
        if (contentType) {
     ...
     * Update content
     */
    async updateContent(id, userId, input) {
        ...
        if (!isAuthor && !isCoAuthor) {
            ...
            try {
                ...
                if (!hasEditorRole) {
                    ...
                    const firestoreUser = await firebase_admin_1.default.firestore().collection('users').doc(userId).get();
     ...
     * Unpublish content
     */
    async unpublishContent(id, userId) {
        const content = await this.prisma.content.findUnique({
            where: { id },
        });
        if (!content) {
     ...
     */
    async getContentStats(authorId) {
     ...
     */
    async restoreVersion(contentId, version, userId) {
