...
app.get('/stats', async (req, res) => {
    try {
        ...
        const endTimestamp = firestore_1.Timestamp.fromDate(end);
        const startTimestamp = start ? firestore_1.Timestamp.fromDate(start) : null;
        let activeUsers = 0;
        let totalVolume = 0;
        let totalPayouts = 0;
        ...
        for (const userDoc of usersSnapshot.docs) {
            ...
            try {
                ...
                if (previousStart && previousEnd) {
                    const prevStartTimestamp = firestore_1.Timestamp.fromDate(previousStart);
                    const prevEndTimestamp = firestore_1.Timestamp.fromDate(previousEnd);
...
app.get('/volume-over-time', async (req, res) => {
    try {
        const startDate = req.query.startDate;
        const endDate = req.query.endDate;
        const currency = req.query.currency;
        ...
        const startTimestamp = firestore_1.Timestamp.fromDate(start);
        const endTimestamp = firestore_1.Timestamp.fromDate(end);
        // Generate month keys
        const months = [];
...
app.get('/currency-distribution', async (req, res) => {
    try {
        const startDate = req.query.startDate;
        const endDate = req.query.endDate;
...
app.listen(PORT, () => {
    ...
    console.log(`[Admin Stats Service] Firebase initialized: ${firebase_admin_1.default.apps.length > 0}`);
