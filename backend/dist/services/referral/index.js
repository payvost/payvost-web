...
class ReferralService {
        ...
        try {
            ...
            if (campaign && campaign.signupBonus) {
                await this.createReward({
                    ...
                    amount: new decimal_js_1.Decimal(campaign.signupBonus.toString()),
     ...
     */
    async processMultiTierReferral(referralId, referrerId, tier) {
        ...
        if (!campaign)
            return;
        const percentage = tier === 'TIER_2' ? campaign.tier2Percentage : campaign.tier3Percentage;
        if (!percentage)
            return;
        // Calculate reward based on direct referral's signup bonus
        const directReward = campaign.signupBonus
            ? new decimal_js_1.Decimal(campaign.signupBonus.toString())
        ...
        if (tierReward.greaterThan(0)) {
            ...
            if (tier === 'TIER_2') {
                await this.processMultiTierReferral(parentReferral.id, parentReferral.referrerId, 'TIER_3');
            }
        }
    }
    /**
     ...
     */
    async processFirstTransaction(userId, transactionAmount, currency) {
        ...
        const campaign = await this.getActiveCampaign();
        if (!campaign || !campaign.firstTxBonus)
            return;
        // Check minimum transaction amount
        if (campaign.firstTxMinAmount &&
            transactionAmount.lessThan(new decimal_js_1.Decimal(campaign.firstTxMinAmount.toString()))) {
            return;
        }
        ...
        await this.createReward({
            ...
            amount: new decimal_js_1.Decimal(campaign.firstTxBonus.toString()),
     ...
     */
    async approveAndPayReward(rewardId, approvedBy) {
        try {
            ...
            await prisma.$transaction(async (tx) => {
                ...
                const rewardAmount = new decimal_js_1.Decimal(reward.amount.toString());
    ...
    async getUserReferralStats(userId) {
        ...
        return {
            ...
            totalEarned: totalEarned._sum.amount || new decimal_js_1.Decimal(0),
    ...
    async createCampaign(data) {
        ...
        if (data.tier2Percentage !== undefined) {
            const tier2 = new decimal_js_1.Decimal(data.tier2Percentage.toString());
        ...
        if (data.tier3Percentage !== undefined) {
            const tier3 = new decimal_js_1.Decimal(data.tier3Percentage.toString());
            if (tier3.lessThan(0) || tier3.greaterThan(100)) {
                throw new Error('Tier 3 percentage must be between 0 and 100');
            }
        }
        // Validate amounts
        if (data.signupBonus !== undefined && new decimal_js_1.Decimal(data.signupBonus.toString()).lessThanOrEqualTo(0)) {
            throw new Error('Signup bonus must be greater than 0');
        }
        if (data.firstTxBonus !== undefined && new decimal_js_1.Decimal(data.firstTxBonus.toString()).lessThanOrEqualTo(0)) {
        ...
        const eligibleCountries = Array.isArray(data.eligibleCountries)
            ? data.eligibleCountries.filter((c) => c && typeof c === 'string' && c.trim().length > 0)
            : (data.eligibleCountries ? [data.eligibleCountries].filter(Boolean) : []);
        const excludedCountries = Array.isArray(data.excludedCountries)
            ? data.excludedCountries.filter((c) => c && typeof c === 'string' && c.trim().length > 0)
        ...
        try {
            const campaign = await prisma.referralCampaign.create({
                data: {
                    ...
                    signupBonus: data.signupBonus ? new decimal_js_1.Decimal(data.signupBonus.toString()) : null,
                    signupCurrency: data.signupCurrency || null,
                    firstTxBonus: data.firstTxBonus ? new decimal_js_1.Decimal(data.firstTxBonus.toString()) : null,
                    firstTxCurrency: data.firstTxCurrency || null,
                    firstTxMinAmount: data.firstTxMinAmount ? new decimal_js_1.Decimal(data.firstTxMinAmount.toString()) : null,
                    tier2Percentage: data.tier2Percentage ? new decimal_js_1.Decimal(data.tier2Percentage.toString()) : null,
                    tier3Percentage: data.tier3Percentage ? new decimal_js_1.Decimal(data.tier3Percentage.toString()) : null,
                    minKycLevel: data.minKycLevel || null,
                    eligibleCountries,
                    excludedCountries,
                    maxReferralsPerUser: data.maxReferralsPerUser || null,
                    maxRewardPerUser: data.maxRewardPerUser ? new decimal_js_1.Decimal(data.maxRewardPerUser.toString()) : null,
                    maxRewardPerCampaign: data.maxRewardPerCampaign ? new decimal_js_1.Decimal(data.maxRewardPerCampaign.toString()) : null,
                    startDate,
                    endDate,
                },
            });
    ...
    async updateCampaign(id, data) {
        ...
        if (data.signupBonus !== undefined)
            updateData.signupBonus = data.signupBonus ? new decimal_js_1.Decimal(data.signupBonus.toString()) : null;
        if (data.signupCurrency !== undefined)
            updateData.signupCurrency = data.signupCurrency || null;
        if (data.firstTxBonus !== undefined)
            updateData.firstTxBonus = data.firstTxBonus ? new decimal_js_1.Decimal(data.firstTxBonus.toString()) : null;
        if (data.firstTxCurrency !== undefined)
            updateData.firstTxCurrency = data.firstTxCurrency || null;
        if (data.firstTxMinAmount !== undefined)
            updateData.firstTxMinAmount = data.firstTxMinAmount ? new decimal_js_1.Decimal(data.firstTxMinAmount.toString()) : null;
        if (data.tier2Percentage !== undefined)
            updateData.tier2Percentage = data.tier2Percentage ? new decimal_js_1.Decimal(data.tier2Percentage.toString()) : null;
        if (data.tier3Percentage !== undefined)
            updateData.tier3Percentage = data.tier3Percentage ? new decimal_js_1.Decimal(data.tier3Percentage.toString()) : null;
        if (data.minKycLevel !== undefined)
            updateData.minKycLevel = data.minKycLevel || null;
        if (data.eligibleCountries !== undefined)
            updateData.eligibleCountries = data.eligibleCountries;
        if (data.excludedCountries !== undefined)
            updateData.excludedCountries = data.excludedCountries;
        if (data.maxReferralsPerUser !== undefined)
            updateData.maxReferralsPerUser = data.maxReferralsPerUser || null;
        if (data.maxRewardPerUser !== undefined)
            updateData.maxRewardPerUser = data.maxRewardPerUser ? new decimal_js_1.Decimal(data.maxRewardPerUser.toString()) : null;
        if (data.maxRewardPerCampaign !== undefined)
            updateData.maxRewardPerCampaign = data.maxRewardPerCampaign ? new decimal_js_1.Decimal(data.maxRewardPerCampaign.toString()) : null;
     ...
     */
    async deleteCampaign(id, hardDelete = false) {
        const campaign = await prisma.referralCampaign.findUnique({
            where: { id },
        });
        if (!campaign) {
    ...
    async getCampaignStats(id) {
        ...
        return {
            ...
            totalRewardsValue: totalRewardsValue._sum.amount || new decimal_js_1.Decimal(0),
            campaign,
        };
    }
}
