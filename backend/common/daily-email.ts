 ...
 */
function is7AMInTimezone(timezone: string): boolean {
  try {
    const now = new Date();
 ...
 */
function getPopularPairs(): string[] {
 ...
 */
async function sendDailyRateEmail(user: { email: string; name?: string | null; country?: string | null }): Promise<boolean> {
  try {
    ...
    const rateSummary = popularPairs.map(code => {
      ...
      const trend: 'up' | 'down' | 'neutral' = change24h > 0.1 ? 'up' : change24h < -0.1 ? 'down' : 'neutral';
 ...
 */
export async function processDailyEmails(): Promise<{ processed: number; sent: number; errors: number }> {
