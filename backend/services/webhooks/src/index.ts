 ...
 */
async function sendEmailNotification(to: string, subject: string, html: string): Promise<void> {
  try {
...
app.post('/reloadly', async (req: Request, res: Response) => {
  try {
    ...
    // Verify signature
    if (!verifyWebhookSignature(bodyText, signature as string, webhookSecret)) {
...
app.listen(PORT, () => {
  ...
  console.log(`[Webhook Service] Firebase initialized: ${admin.apps.length > 0}`);
